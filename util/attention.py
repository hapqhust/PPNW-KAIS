...
## namedtuple(class_name, (a, b)) : quite way to create class with 'self.a=a ; self.b=b'
AttentionOutput = namedtuple("AttentionOutput", ['weight', 'output'])


'''
used in class ApplyAttentionMemory 
To mask memory and leave "input_neighborhood_lengths"
(mask_length = input_neighborhood_lengths)
'''
class MemoryMask(snt.AbstractModule):
    ...
    def _build(self, inputs, mask_length, maxlen=None,):
        ...
        # [batch_size, length]
        # any mask_length>maxlen, will be masked as False  
        #   e.g. tf.sequence_mask(1, 3) ==> [True, False, False]
        ...
        ## tf.control_dependencies(control_inputs): A list of Operation or Tensor 
        ##   executed before running the context.
        ## for why using tf.control_dependencies(op/tensor):
        ...
        with tf.control_dependencies([tf.assert_positive(
            ...
            # Take the min(input, lower_bound) in return
...
'''
used in class VariableLengthMemoryLayer
==> item's neighbors + mask_length + nn.softmax + weighted output Matrix_C
'''
            ...
            if hop_k > 0:
                ...
                with tf.name_scope('Map'):
                    ...
                    tf.logging.info('Creating Hop Mapping {} with {}'.format(hop_k+1,
                                                                             self._activation_fn))
            # Apply attention
            hop = ApplyAttentionMemory(name='AttentionHop%s' % hop_k)
