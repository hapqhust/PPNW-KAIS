...
# def _bpr_loss(positive, negative, pos_w, neg_w, latent_div_score, name=None):
...
#         loss_weight = 1 + gamma * (pos_w - neg_w)
        
#         # clip the value in case neg_w is too big (results in negative loss)
#         loss_weight = tf.clip_by_value(loss_weight, clip_value_min=0., clip_value_max=np.inf)
        
#         loss = loss_weight * (- tf.log(tf.nn.sigmoid(difference) + eps))
        
#         return tf.reduce_mean(loss, name=scope)

#########################   Customized Gradient function   ##########################
@tf.custom_gradient
def _bpr_loss(positive, negative, pos_w, neg_w, name=None):
    ...
    with tf.name_scope(name, 'BPRLoss', [positive, negative]) as scope:
        ...
        pop_gamma = 5  #### ->1 when BN is on.
        
        # Normalization within a mini-batch
        #    (pos_w - neg_w) to be (mean, std) -> (0, 1)
        pop_weights = (pos_w - neg_w)
        
#         mean, var = tf.nn.moments(pop_weights, axes=[0])  #### Normalization
#         pop_weights = (pop_weights - mean) / tf.sqrt(var)  #### Normalization

        loss_weight = 1 + pop_gamma * pop_weights
        
        # clip the value in case neg_w is too big (results in negative loss_weight)
        loss_weight = tf.clip_by_value(loss_weight, clip_value_min=0., clip_value_max=np.inf)
        
        loss = loss_weight * (- tf.log(tf.nn.sigmoid(difference) + eps))

        def grad(dy):   
            pos_grad = tf.gradients(loss, positive)
            neg_grad = tf.gradients(loss, negative)

            return pos_grad, neg_grad, tf.zeros_like(pos_w), tf.zeros_like(neg_w)
        

        return tf.reduce_mean(loss, name=scope), grad
...
#########################   Plain AllRank-Pop   #########################
'''
This plain loss function is simply the Pointwise loss function with ONLY Positive Items.
    - No weight
    - Only Positive item
    - Pointwise
Remember the set the Neg_Ratio=1
'''
...
#         loss = tf.math.square(positive - 1)
...
class LossLayer(snt.AbstractModule):
    ...
    # _build() : required by snt.AbstractModule
    def _build(self, X, y, pos_w, neg_w):
        """
        :param X: pos item score
        :param y: neg item score
        :returns: Loss with l1/l2 regularization added if in keys
        """
        graph_regularizers = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)

        self._loss = tf.squeeze(_bpr_loss(X, y, pos_w, neg_w))
        ...
        if graph_regularizers:
            ...
            # 'LOSS_REG' & 'LOSS_NO_REG' are not default tf keys. created in helper.GraphKey
...
class OptimizerLayer(snt.AbstractModule):
    ...
    def _build(self, loss, trainable_variables=None):
        ...
        Input:
            loss: tensor
        Return:
            optimizatio op == (compute_gradients + apply_gradients)
        """
